...
bool draw_image( const int index, const unsigned char* data, const int data_size, const ImVec2 pos, const ImVec2 size, const ImColor color )
{
...
bool draw_weapon_icon( sdk::c_tf_player* player, ImVec2 position, ImVec2 size )
{
#define DRAW( name ) draw_image( HASH( #name ), name.data( ), name.size( ), position, size, ImColor( 255, 255, 255, 255 ) )

	if ( !player )
		return false;

	const auto weapon_handle = player->active_weapon( );

	if ( !weapon_handle.index )
		return false;

	const auto weapon = reinterpret_cast< sdk::c_tf_weapon_base* >( g_interfaces->entity_list->get_client_entity_from_handle( weapon_handle ) );
	...
	case sdk::tf_weapon_scattergun:
		return DRAW( icon_scattergun );
	case sdk::tf_weapon_pep_brawler_blaster:
		return DRAW( icon_baby_face_s_blaster );
	case sdk::tf_weapon_soda_popper:
		return DRAW( icon_soda_popper );
	case sdk::tf_weapon_handgun_scout_primary:
		return DRAW( icon_shortstop );
	case sdk::tf_weapon_pistol_scout:
		return DRAW( icon_pistol );
	case sdk::tf_weapon_handgun_scout_secondary:
		return DRAW( icon_pretty_boy_s_pocket_pistol );
	case sdk::tf_weapon_cleaver:
		return DRAW( icon_flying_guillotine );
...
void render_visuals( sdk::c_tf_player* entity )
{
	...
	if ( *visuals_player_name ) {
		const auto name_size = g_verdana_bd_11->CalcTextSizeA( 11.f, FLT_MAX, -1.f, entity->name( ).c_str( ) );

		draw->AddText( g_verdana_bd_11, 11.f, ImVec2( ( bb.x + bb.z ) / 2.f - name_size.x / 2.f + 1, bb.y - name_size.y - 1 ),
		               ImColor( *visuals_player_name_outline_color ), entity->name( ).c_str( ) );
		draw->AddText( g_verdana_bd_11, 11.f, ImVec2( ( bb.x + bb.z ) / 2.f - name_size.x / 2.f, bb.y - name_size.y - 2 ),
	...
	if ( *visuals_player_health_text ) {
		...
		if ( *visuals_player_health_text_minimum >= health - max_health ) {
			...
			const auto text_size = g_verdana_bd_11->CalcTextSizeA( 11.f, FLT_MAX, -1.f, std::to_string( static_cast< int >( health ) ).c_str( ) );

			if ( *visuals_player_health_bar ) {
				draw->AddText(
					g_verdana_bd_11, 11.f,
	...
	for ( const auto& indicator : text ) {
		const auto text_size = g_verdana_bd_11->CalcTextSizeA( 11.f, FLT_MAX, -1.f, indicator.first.c_str( ) );

		draw->AddText( g_verdana_bd_11, 11.f, ImVec2( bb.z + 2, bb.y + offset + 1 ), indicator.second.first, indicator.first.c_str( ) );
		draw->AddText( g_verdana_bd_11, 11.f, ImVec2( bb.z + 2, bb.y + offset ), indicator.second.second, indicator.first.c_str( ) );
