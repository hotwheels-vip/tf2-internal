...
void __fastcall run_command::detour( void* ecx, void* edx, sdk::c_base_player* player, sdk::c_user_cmd* command, void* move_helper )
{
	...
	const auto ping_ticks       = g_interfaces->engine_client->get_net_channel_info( )->get_latency( 0 ) / g_interfaces->globals->interval_per_tick;
	const auto ideal_final_tick = g_tickbase->last_tick + correction_ticks + ping_ticks;
	...
	if ( estimated_final_tick > too_fast_limit || estimated_final_tick < too_slow_limit ) {
		const auto corrected_tick = ideal_final_tick - simulation_ticks + g_interfaces->globals->sim_ticks_this_frame;
