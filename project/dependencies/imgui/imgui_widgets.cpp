...
static ImVec2 InputTextCalcTextSizeW( const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining = NULL, ImVec2* out_offset = NULL,
...
#	endif
	...
	// AllowOverlap mode (rarely used) requires previous frame HoveredId to be null or to match. This allows using patterns where a later submitted
	...
	if ( hovered ) {
		if ( !( flags & ImGuiButtonFlags_NoKeyModifiers ) || ( !g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt ) ) {
			...
			if ( ( flags & ImGuiButtonFlags_MouseButtonLeft ) && IsMouseClicked( 0, test_owner_id ) ) {
				mouse_button_clicked = 0;
			} else if ( ( flags & ImGuiButtonFlags_MouseButtonRight ) && IsMouseClicked( 1, test_owner_id ) ) {
				mouse_button_clicked = 1;
			} else if ( ( flags & ImGuiButtonFlags_MouseButtonMiddle ) && IsMouseClicked( 2, test_owner_id ) ) {
			...
			if ( flags & ImGuiButtonFlags_PressedOnRelease ) {
				...
				if ( ( flags & ImGuiButtonFlags_MouseButtonLeft ) && IsMouseReleased( 0, test_owner_id ) ) {
					mouse_button_released = 0;
				} else if ( ( flags & ImGuiButtonFlags_MouseButtonRight ) && IsMouseReleased( 1, test_owner_id ) ) {
					mouse_button_released = 1;
				} else if ( ( flags & ImGuiButtonFlags_MouseButtonMiddle ) && IsMouseReleased( 2, test_owner_id ) ) {
			...
			// Relies on repeat logic of IsMouseClicked() but we may as well do it ourselves if we end up exposing finer RepeatDelay/RepeatRate
			// settings.
...
bool ImGui::ButtonEx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags )
{
	...
	text_animation.Update( hovered || g.ActiveId == id ? 4.f : -4.f, 1.f, 0.5f, 1.f );
	...
	RenderTextClipped( bb.Min + style.FramePadding + ImVec2( 3.f, 0.f ), bb.Max - style.FramePadding + ImVec2( 3.f, 0.f ), label, NULL, &label_size,
...
bool ImGui::ArrowButtonEx( const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags )
{
	...
	RenderArrow( window->DrawList, bb.Min + ImVec2( ImMax( 0.0f, ( size.x - g.FontSize ) * 0.5f ), ImMax( 0.0f, ( size.y - g.FontSize ) * 0.5f ) ),
	             text_col, dir );
...
bool ImGui::CloseButton( ImGuiID id, const ImVec2& pos )
{
	...
	// Tweak 1: Shrink hit-testing area if button covers an abnormally large proportion of the visible region. That's in order to facilitate moving
	// the window away. (#3825) This may better be applied as a general hit-rect reduction mechanism for all widgets to ensure the area to move window
...
ImRect ImGui::GetWindowScrollbarRect( ImGuiWindow* window, ImGuiAxis axis )
{
	...
	// IM_ASSERT(scrollbar_size > 0.0f); /* LOL, this is stoopid */
	if ( axis == ImGuiAxis_X )
		return ImRect( inner_rect.Min.x, ImMax( outer_rect.Min.y, outer_rect.Max.y - border_size - scrollbar_size ), inner_rect.Max.x,
		               outer_rect.Max.y );
	else
		return ImRect( ImMax( outer_rect.Min.x, outer_rect.Max.x - border_size - scrollbar_size ), inner_rect.Min.y, outer_rect.Max.x,
		               inner_rect.Max.y );
}
...
void ImGui::Scrollbar( ImGuiAxis axis )
{
	...
	ScrollbarEx( ImRect( bb.Min + ImVec2( 2.f, 0.f ), bb.Max + ImVec2( 2.f, 0.f ) ), id, axis, &scroll, ( ImS64 )size_avail, ( ImS64 )size_contents,
	             rounding_corners );
                         ...
                         ImDrawFlags flags )
{
	...
	// When we are too small, start hiding and disabling the grab (this reduce visual noise on very small window and facilitate using the window
	// resize grab)
	float alpha = 1.0f;
	...
	if ( held && allow_interaction && grab_h_norm < 1.0f ) {
		...
		// It is ok to modify Scroll here because we are being called in Begin() after the calculation of ContentSize and before setting up our
		// starting position
...
bool ImGui::ImageButton( const char* str_id, ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1,
...
void ImGui::RenderSettingsPopup( const char* str_id, const std::function< void( ) >& function )
{
	const auto hashed_id = ImHashStr( str_id );

	SameLine( GetContentRegionAvail( ).x - 10.f - first_icon_14->CalcTextSizeA( first_icon_14->FontSize, 0.f, FLT_MAX, ICON_FA_COG ).x / 2.f );

	auto text_animation = ImAnimationHelper( hashed_id, GetIO( ).DeltaTime );

	const ImColor text_color = GetColorU32( ImGuiCol_Text );

	PushFont( first_icon_14 );
	TextColored( ImColor::Blend( ImVec4( text_color.Value.x, text_color.Value.y, text_color.Value.z, text_color.Value.w * 0.5f ),
	                             ImColor( 200 / 255.f, 255 / 255.f, 80 / 255.f, 1.f ), text_animation.AnimationData->second ),
	             ICON_FA_COG );
	PopFont( );

	const bool hovered = IsItemHovered( );

	if ( hovered && IsMouseClicked( ImGuiMouseButton_::ImGuiMouseButton_Right ) )
		OpenPopup( str_id );

	const bool popup_open = IsPopupOpen( str_id );

	text_animation.Update( popup_open || hovered ? 4.f : -4.f );

	auto popup_animation = ImAnimationHelper( hashed_id + 1u, GetIO( ).DeltaTime );
	popup_animation.Update( popup_open ? 10.f : -10.f );

	[ & ]( ) {
		PushStyleColor( ImGuiCol_::ImGuiCol_PopupBg, GetColorU32( ImGuiCol_::ImGuiCol_ChildBg ) );
		PushStyleColor( ImGuiCol_::ImGuiCol_Border, GetColorU32( ImGuiCol_::ImGuiCol_WindowBg ) );
		PushStyleVar( ImGuiStyleVar_::ImGuiStyleVar_PopupBorderSize, 1.f );

		SetNextWindowSize( ImVec2( 150.f, -1 ) * ImVec2( popup_animation.AnimationData->second, popup_animation.AnimationData->second ) );
...
void ImGui::ImageWithText( const char* label, int* selected_module, int wanted_module, ImTextureID user_texture_id, const ImVec2& size,
                           const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col )
{
	...
	const auto draw_pos = ImGui::GetCursorScreenPos( );

	window->DrawList->AddImageRounded( user_texture_id, bb.Min, bb.Max, uv0, uv1, GetColorU32( tint_col ), 25.f,
	                                   ImDrawFlags_::ImDrawFlags_RoundCornersTop );

	const bool hovered = ImGui::IsItemHovered( );

	if ( hovered && ImGui::IsMouseClicked( ImGuiMouseButton_::ImGuiMouseButton_Left ) )
		*selected_module = wanted_module;

	auto animation = ImAnimationHelper( ImHashStr( label ), ImGui::GetIO( ).DeltaTime );
	animation.Update( 1.f, hovered ? 4.f : -4.f );

	window->DrawList->AddRectFilledMultiColor( bb.Min, bb.Max, ImColor( 0.f, 0.f, 0.f, animation.AnimationData->second * 0.f ),
	                                           ImColor( 0.f, 0.f, 0.f, animation.AnimationData->second * 0.f ),
	                                           ImColor( 0.f, 0.f, 0.f, animation.AnimationData->second * 0.90f ),
	                                           ImColor( 0.f, 0.f, 0.f, animation.AnimationData->second * 0.90f ) );

	// window->DrawList->AddText(gilroy_14, gilroy_14->FontSize, ImVec2(bb.Min.x + 5.f, draw_pos.y - gilroy_14->CalcTextSizeA(gilroy_14->FontSize,
	// FLT_MAX, 0.f, label).y - 10.f), ImColor(1.f, 1.f, 1.f, animation.AnimationData->second), label);
}

#	ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
// Legacy API obsoleted in 1.89. Two differences with new ImageButton()
// - new ImageButton() requires an explicit 'const char* str_id'    Old ImageButton() used opaque imTextureId (created issue with: multiple buttons
...
// If you need to change padding with new ImageBILL utton() you can use PushStyleVar(ImGuiStyleVar_FramePadding, value), consistent with other Button
// functions.
bool ImGui::ImageButton( ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding,
                         const ImVec4& bg_col, const ImVec4& tint_col )
{
	ImGuiContext& g     = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
...
bool ImGui::Checkbox( const char* label, bool* v )
{
	...
	const ImVec2 pos      = window->DC.CursorPos;
	const ImRect total_bb( pos, pos + ImVec2( square_sz + ( label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f ) + 2.f,
	...
	auto frame_animation = ImAnimationHelper( id, ImGui::GetIO( ).DeltaTime );
	frame_animation.Update( *v ? 5.f : -5.f, 1.f, 0.f, 1.f );
	...
	PushStyleColor( ImGuiCol_Text,
	                ImVec4( text_color.Value.x, text_color.Value.y, text_color.Value.z, text_color.Value.w * text_animation.AnimationData->second ) );
	ImVec2 label_pos = ImVec2( check_bb.Max.x + style.ItemInnerSpacing.x + 2.f, check_bb.Min.y + style.FramePadding.y - 1.f );
	if ( g.LogEnabled )
		LogRenderedText( &label_pos, ( g.LastItemData.InFlags & ImGuiItemFlags_MixedValue ) != 0 ? "[~]" : *v ? "[x]" : "[ ]" );
	if ( label_size.x > 0.0f )
...
// FIXME: This would work nicely if it was a public template, e.g. 'template<T> RadioButton(const char* label, T* v, T v_button)', but I'm not sure
// how we would expose it..
...
void ImGui::SeparatorEx( ImGuiSeparatorFlags flags )
{
	...
	if ( flags & ImGuiSeparatorFlags_Vertical ) {
		// Vertical separator, for menu bars (use current line height). Not exposed because it is misleading and it doesn't have an effect on regular
		// layout.
...
bool ImGui::SplitterBehavior( const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2,
...
void ImGui::ShrinkWidths( ImGuiShrinkWidthItem* items, int count, float width_excess )
{
	...
	// Ensure that e.g. the right-most tab of a shrunk tab-bar always reaches exactly at the same distance from the right-most edge of the tab bar
	// separator.
	width_excess = 0.0f;
...
bool ImGui::BeginCombo( const char* label, const char* preview_value, ImGuiComboFlags flags )
{
	...
	bool popup_open = IsPopupOpen( id, ImGuiPopupFlags_::ImGuiPopupFlags_None );
	...
	if ( !( flags & ImGuiComboFlags_NoArrowButton ) ) {
		auto RenderArrow = [ & ]( ImVec2 pos, ImU32 col ) {
			const float h = 15 * 1.00f;
			float r       = h * 0.40f * 1.f;

			ImVec2 center = pos + ImVec2( h * 0.50f, h * 0.50f * 1.f );

			ImVec2 a = { }, b = { }, c = { };

			auto arrow_animation = ImAnimationHelper( id, ImGui::GetIO( ).DeltaTime );
			arrow_animation.Update( popup_open ? 50.f : -50.f, 1.f, -r, r );

			a = ImVec2( +0.00f, +0.350f ) * arrow_animation.AnimationData->second;
			b = ImVec2( -0.666f, -0.350f ) * arrow_animation.AnimationData->second;
			c = ImVec2( +0.666f, -0.350f ) * arrow_animation.AnimationData->second;

			window->DrawList->AddLine( center + a, center + b, col );
			window->DrawList->AddLine( center + a, center + c, col );
		};
	...
	auto text_animation = ImAnimationHelper( id + ImHashStr( "text-animation" ), ImGui::GetIO( ).DeltaTime );
	text_animation.Update( hovered || popup_open ? 4.f : -4.f, 1.f, 0.5f, 1.f );
	...
	if ( preview_value != NULL && !( flags & ImGuiComboFlags_NoPreview ) ) {
		...
		RenderTextClipped( frame_bb.Min + style.FramePadding + ImVec2( 4.f, 1.f ), ImVec2( value_x2, frame_bb.Max.y ), preview_value, NULL, NULL );
	}

	if ( label_size.x > 0 )
		RenderText( ImVec2( frame_bb.Min.x - 1.f, total_bb.Min.y + style.FramePadding.y - 4.f ), label );
	...
	if ( ImGuiWindow* popup_window = FindWindowByName( name ) )
		if ( popup_window->WasActive ) {
			...
			SetNextWindowPos( pos + ImVec2( 0.f, 1.f ) );
		}
...
bool ImGui::MultiCombo( const char* label, std::vector< bool >& values, const std::vector< const char* >& item_list, int items_count )
{
	...
	const float w           = CalcItemWidth( ) - ( label_size.x > 0.0f ? style.ItemInnerSpacing.x + GetFrameHeight( ) : 0.0f );

	std::vector< std::string_view > active_items = { };

	for ( int i = 0; i < items_count; i++ ) {
		if ( values[ i ] )
			active_items.push_back( item_list[ i ] );
	}
	...
	if ( buffer.empty( ) )
		...
		const char* wrap_pos = g.Font->CalcWordWrapPositionA( GetCurrentWindow( )->FontWindowScale, &buffer[ 0 ], buffer.data( ) + buffer.length( ),
		                                                      w - style.FramePadding.x * 2.0f );
		const std::size_t wrap_len = wrap_pos - &buffer[ 0 ];

		if ( wrap_len > 0U && wrap_len < buffer.length( ) ) {
			buffer.resize( wrap_len );
			buffer.append( "..." );
		}
	}
...
bool ImGui::Keybind( const char* label, KeyBind_t* key_data, bool has_style )
{
	...
	if ( key_data->iKey != 0 && g.ActiveId != id )
		strcpy_s( buf_display, key_names[ key_data->iKey ] );
	else if ( g.ActiveId == id )
		strcpy_s( buf_display, ( "[...]" ) );

	ImGui::SameLine( window->Size.x - 18.f - CalcTextSize( buf_display ).x );

	const ImVec2 label_size = CalcTextSize( key_names[ key_data->iKey ] );
	const ImRect frame_bb( ImVec2( window->DC.CursorPos.x, window->DC.CursorPos.y ),
	                       ImVec2( window->DC.CursorPos.x, window->DC.CursorPos.y ) - label_size );

	const ImRect total_bb( ImVec2( window->DC.CursorPos.x - 8.f, window->DC.CursorPos.y ),
	                       ImVec2( window->DC.CursorPos.x - 8.f, window->DC.CursorPos.y ) +
	                           ImVec2( window->Pos.x + window->Size.x - window->DC.CursorPos.x, label_size.y ) );
	...
	if ( hovered && io.MouseClicked[ 0 ] ) {
		...
		if ( g.ActiveId == id )
			ClearActiveID( );
	} else if ( hovered && io.MouseClicked[ 1 ] && has_style )
		OpenPopup( label );
	...
	PushStyleVar( ImGuiStyleVar_::ImGuiStyleVar_WindowPadding, ImVec2( 0.f, 8.f ) );

	SetNextWindowSize( ImVec2( 61.f, 65.f ), ImGuiCond_Always );
	SetNextWindowPos( ImVec2( frame_bb.Max.x + 5.f, frame_bb.Max.y ), ImGuiCond_Always );

	if ( BeginPopup( label, ImGuiWindowFlags_NoMove ) ) {
		const auto popup_window = ImGui::GetCurrentWindow( );

		const auto position = popup_window->Pos;

		popup_window->DrawList->AddRect( position, position + ImGui::GetWindowSize( ), ImColor( 50, 50, 50, 100 ), style.WindowRounding );

		if ( Selectable( "always on", key_data->iKeyStyle == EKeyStyles::KEY_STYLE_ALWAYS_ON,
		                 ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups ) )
			key_data->iKeyStyle = EKeyStyles::KEY_STYLE_ALWAYS_ON;

		if ( Selectable( "hold", key_data->iKeyStyle == EKeyStyles::KEY_STYLE_HOLD, ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups ) )
			key_data->iKeyStyle = EKeyStyles::KEY_STYLE_HOLD;

		if ( Selectable( "toggle", key_data->iKeyStyle == EKeyStyles::KEY_STYLE_TOGGLE,
		                 ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups ) )
			key_data->iKeyStyle = EKeyStyles::KEY_STYLE_TOGGLE;
...
bool ImGui::CurveEditor( const char* label, Points_t* points, ImVec2 data[], int members )
{
	IM_ASSERT( members >= 4 );      // Mathamtically impossible for use to make a curve
	IM_ASSERT( points != nullptr ); // Points must be valid

	// Make an empty box where we will put our curve editor
	ImGuiWindow* window = GetCurrentWindow( );
	if ( window->SkipItems )
		return false;
	...
	// Draw the points
	const auto point_position_a = ImVec2( frame_bb.Min.x + ( frame_bb.Max.x - frame_bb.Min.x ) * points->PointA.x,
	                                      frame_bb.Min.y + ( frame_bb.Max.y - frame_bb.Min.y ) * ( 1.f + -( points->PointA.y ) ) );
	const auto point_position_b = ImVec2( frame_bb.Min.x + ( frame_bb.Max.x - frame_bb.Min.x ) * points->PointB.x,
	                                      frame_bb.Min.y + ( frame_bb.Max.y - frame_bb.Min.y ) * ( 1.f + -( points->PointB.y ) ) );

	const auto point_size = 10.f;

	// Fix point position clipping off of the curve editor
	const auto point_position_a_clamped = ImVec2( ImClamp( point_position_a.x, frame_bb.Min.x + point_size, frame_bb.Max.x - point_size ),
	                                              ImClamp( point_position_a.y, frame_bb.Min.y + point_size, frame_bb.Max.y - point_size ) );
	const auto point_position_b_clamped = ImVec2( ImClamp( point_position_b.x, frame_bb.Min.x + point_size, frame_bb.Max.x - point_size ),
	...
	const auto point_a_bb =
		ImRect( point_position_a_clamped - ImVec2( point_size, point_size ), point_position_a_clamped + ImVec2( point_size, point_size ) );
	const auto point_b_bb =
		ImRect( point_position_b_clamped - ImVec2( point_size, point_size ), point_position_b_clamped + ImVec2( point_size, point_size ) );
	point_a_animation.Update( 2.f, ImGui::IsMouseHoveringRect( point_a_bb.Min, point_a_bb.Max, false ) ? 2.f : -2.f, 0.5f );
	point_b_animation.Update( 2.f, ImGui::IsMouseHoveringRect( point_b_bb.Min, point_b_bb.Max, false ) ? 2.f : -2.f, 0.5f );

	// Add button behavior to each point
	auto point_a_hovered = false;
	auto point_b_hovered = false;

	auto point_a_held = false;
	auto point_b_held = false;

	const auto point_a_id = window->GetID( "point_a" );
	const auto point_b_id = window->GetID( "point_b" );

	if ( !ItemAdd( point_a_bb, point_a_id, &point_a_bb ) )
		return false;

	if ( !ItemAdd( point_b_bb, point_b_id, &point_b_bb ) )
		return false;

	ButtonBehavior( point_a_bb, point_a_id, &point_a_hovered, &point_a_held );
	ButtonBehavior( point_b_bb, point_b_id, &point_b_hovered, &point_b_held );

	if ( point_a_held ) {
		points->PointA.x = ( io.MousePos.x - frame_bb.Min.x ) / ( frame_bb.Max.x - frame_bb.Min.x );
		points->PointA.y = ( frame_bb.Max.y - io.MousePos.y ) / ( frame_bb.Max.y - frame_bb.Min.y );
	}

	if ( point_b_held ) {
		points->PointB.x = ( io.MousePos.x - frame_bb.Min.x ) / ( frame_bb.Max.x - frame_bb.Min.x );
		points->PointB.y = ( frame_bb.Max.y - io.MousePos.y ) / ( frame_bb.Max.y - frame_bb.Min.y );
	}

	// Bound points to frame
	points->PointA.x = ImClamp( points->PointA.x, 0.0f, 1.0f );
	points->PointA.y = ImClamp( points->PointA.y, 0.0f, 1.0f );
	...
	for ( int i = 0; i < members; i++ ) {
		...
		const auto pos  = ImBezierCubicCalc( ImVec2( 0.f, 0.f ), points->PointA, points->PointB, ImVec2( 1.f, 1.f ), time );

		const auto point_position = ImVec2( frame_bb.Min.x + ( frame_bb.Max.x - frame_bb.Min.x ) * pos.x,
		                                    frame_bb.Min.y + ( frame_bb.Max.y - frame_bb.Min.y ) * ( 1.f + -pos.y ) );

		data[ i ] = pos;

		if ( i > 0 ) {
			const auto prev_time = ( i - 1 ) / ( float )( members - 1 );

			const auto prev_pos = ImBezierCubicCalc( ImVec2( 0.f, 0.f ), points->PointA, points->PointB, ImVec2( 1.f, 1.f ), prev_time );
...
bool ImGui::DragBehaviorT( ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format,
                           ImGuiSliderFlags flags )
{
	...
	// Inputs accumulates into g.DragCurrentAccum, which is flushed into the current value as soon as it makes a difference with our precision
	// settings
	...
	// Avoid altering values and clamping when we are _already_ past the limits and heading in the same direction, so e.g. if range is 0..255, current
	...
	if ( is_logarithmic ) {
		// Convert to parametric space, apply delta, convert back
		float v_new_parametric = ScaleRatioFromValueT< TYPE, SIGNEDTYPE, FLOATTYPE >( data_type, v_cur, v_min, v_max, is_logarithmic,
		                                                                              logarithmic_zero_epsilon, zero_deadzone_halfsize );
                          ...
                          ImGuiSliderFlags flags )
{
	...
	IM_ASSERT( ( flags == 1 || ( flags & ImGuiSliderFlags_InvalidMask_ ) == 0 ) &&
	           "Invalid ImGuiSliderFlags flags! Has the 'float power' argument been mistakenly cast to flags? Call function with "
...
bool ImGui::DragScalar( const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max,
                        const char* format, ImGuiSliderFlags flags )
{
	...
	auto frame_animation = ImAnimationHelper( id, ImGui::GetIO( ).DeltaTime );
	frame_animation.Update( g.ActiveId == id ? 4.f : -4.f, 1.f, 0.f, 1.f );

	RenderFrame( frame_bb.Min, frame_bb.Max, ImColor( 25 / 255.f, 25 / 255.f, 25 / 255.f, 1.f ), true, style.FrameRounding );
	RenderFrame( frame_bb.Min, frame_bb.Max, ImColor( 200 / 255.f, 255 / 255.f, 80 / 255.f, 1.f * frame_animation.AnimationData->second ), true,
	             style.FrameRounding );
...
bool ImGui::DragScalarN( const char* label, ImGuiDataType data_type, void* p_data, int components, float v_speed, const void* p_min,
...
bool ImGui::DragFloatRange2( const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max,
	...
	if ( is_logarithmic ) {
		...
		else if ( ( v_min * v_max ) < 0.0f ) // Range crosses zero, so split into two portions
		{
			float zero_point_center =
				( -( float )v_min ) /
				( ( float )v_max -
			      ( float )v_min ); // The zero point in parametric space.  There's an argument we should take the logarithmic nature into account
			                        // when calculating this, but for now this should do (and the most common case of a symmetrical range works fine)
...
bool ImGui::SliderBehaviorT( const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format,
                             ImGuiSliderFlags flags, ImRect* out_grab_bb, int* percent )
{
	ImGuiContext& g         = *GImGui;
	const ImGuiStyle& style = g.Style;
	...
	*v = ImClamp( *v, v_min, v_max );
	...
	if ( slider_sz < 1.0f ) {
		...
		*percent = floorf( ( float )( *v - v_min ) / ( float )( v_max - v_min ) * 100.f );
		...
		if ( axis == ImGuiAxis_X )
			*out_grab_bb =
				ImRect( slider_usable_pos_min - grab_sz * 0.5f, bb.Min.y + grab_padding, grab_pos + grab_sz * 0.5f, bb.Max.y - grab_padding );
		else
			*out_grab_bb =
				ImRect( bb.Min.x + grab_padding, grab_pos - grab_sz * 0.5f, bb.Max.x - grab_padding, slider_usable_pos_min + grab_sz * 0.5f );
	}
...
bool ImGui::SliderBehavior( const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* p_v, const void* p_min, const void* p_max,
                            const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb, int* percent )
{
	// Read imgui.cpp "API BREAKING CHANGES" section for 1.78 if you hit this assert.
	IM_ASSERT( ( flags == 1 || ( flags & ImGuiSliderFlags_InvalidMask_ ) == 0 ) &&
	...
	case ImGuiDataType_S16: {
		ImS32 v32 = ( ImS32 ) * ( ImS16* )p_v;
		bool r    = SliderBehaviorT< ImS32, ImS32, float >( bb, id, ImGuiDataType_S32, &v32, *( const ImS16* )p_min, *( const ImS16* )p_max, format,
                                                         flags, out_grab_bb, percent );
		if ( r )
			*( ImS16* )p_v = ( ImS16 )v32;
		return r;
	}
	case ImGuiDataType_U16: {
		ImU32 v32 = ( ImU32 ) * ( ImU16* )p_v;
		bool r    = SliderBehaviorT< ImU32, ImS32, float >( bb, id, ImGuiDataType_U32, &v32, *( const ImU16* )p_min, *( const ImU16* )p_max, format,
                                                         flags, out_grab_bb, percent );
		if ( r )
			*( ImU16* )p_v = ( ImU16 )v32;
		return r;
	...
	case ImGuiDataType_S32:
		...
		return SliderBehaviorT< ImS32, ImS32, float >( bb, id, data_type, ( ImS32* )p_v, *( const ImS32* )p_min, *( const ImS32* )p_max, format,
		                                               flags, out_grab_bb, percent );
	case ImGuiDataType_U32:
		IM_ASSERT( *( const ImU32* )p_max <= IM_U32_MAX / 2 );
		return SliderBehaviorT< ImU32, ImS32, float >( bb, id, data_type, ( ImU32* )p_v, *( const ImU32* )p_min, *( const ImU32* )p_max, format,
		                                               flags, out_grab_bb, percent );
	case ImGuiDataType_S64:
		IM_ASSERT( *( const ImS64* )p_min >= IM_S64_MIN / 2 && *( const ImS64* )p_max <= IM_S64_MAX / 2 );
		return SliderBehaviorT< ImS64, ImS64, double >( bb, id, data_type, ( ImS64* )p_v, *( const ImS64* )p_min, *( const ImS64* )p_max, format,
		                                                flags, out_grab_bb, percent );
	case ImGuiDataType_U64:
		IM_ASSERT( *( const ImU64* )p_max <= IM_U64_MAX / 2 );
		return SliderBehaviorT< ImU64, ImS64, double >( bb, id, data_type, ( ImU64* )p_v, *( const ImU64* )p_min, *( const ImU64* )p_max, format,
		                                                flags, out_grab_bb, percent );
	case ImGuiDataType_Float:
		IM_ASSERT( *( const float* )p_min >= -FLT_MAX / 2.0f && *( const float* )p_max <= FLT_MAX / 2.0f );
		return SliderBehaviorT< float, float, float >( bb, id, data_type, ( float* )p_v, *( const float* )p_min, *( const float* )p_max, format,
		                                               flags, out_grab_bb, percent );
	case ImGuiDataType_Double:
                          ...
                          ImGuiSliderFlags flags )
{
	...
	const bool can_add = ItemAdd( total_bb, id, &frame_bb, temp_input_allowed ? ImGuiItemFlags_Inputable : 0 );
	if ( !can_add )
		return false;
	...
	decltype( old_value )::iterator old_value_data       = old_value.find( id );
	decltype( active_value )::iterator active_value_data = active_value.find( id );

	constexpr float travel_amount = 0.1f;

	if ( old_value_data == old_value.end( ) ) {
		old_value.insert( { id, 0 } );
		old_value_data = old_value.find( id );
	}
	...
	if ( old_value_data->second != active_value_data->second ) {
		if ( old_value_data->second < active_value_data->second )
			old_value_data->second += ( active_value_data->second - old_value_data->second ) / 13 + travel_amount;
		if ( old_value_data->second > active_value_data->second )
			old_value_data->second -= ( old_value_data->second - active_value_data->second ) / 13 + travel_amount;
	}
	...
	auto text_animation = ImAnimationHelper( id + 2u, ImGui::GetIO( ).DeltaTime );
	text_animation.Update( hovered || g.ActiveId == id ? 4.f : -4.f, 1.f, 0.5f, 1.f );
...
bool ImGui::SliderScalarN( const char* label, ImGuiDataType data_type, void* v, int components, const void* v_min, const void* v_max,
                           const char* format, ImGuiSliderFlags flags )
{
	ImGuiWindow* window = GetCurrentWindow( );
	if ( window->SkipItems )
                           ...
                           const char* format, ImGuiSliderFlags flags )
{
	...
	const bool value_changed =
		SliderBehavior( frame_bb, id, data_type, p_data, p_min, p_max, format, flags | ImGuiSliderFlags_Vertical, &grab_bb, &percent );
	if ( value_changed )
		MarkItemEdited( id );
...
const char* ImParseFormatFindEnd( const char* fmt )
{
	...
	const unsigned int ignored_lowercase_mask = ( 1 << ( 'h' - 'a' ) ) | ( 1 << ( 'j' - 'a' ) ) | ( 1 << ( 'l' - 'a' ) ) | ( 1 << ( 't' - 'a' ) ) |
	                                            ( 1 << ( 'w' - 'a' ) ) | ( 1 << ( 'z' - 'a' ) );
...
// - For scanning we need to remove all width and precision fields "%3.7f" -> "%f". BUT don't strip types like "%I64d" which includes digits. !
// "%07I64d" -> "%I64d"
                         ...
                         ImGuiInputTextFlags flags )
{
	...
	if ( g.ActiveId == 0 &&
	     ( flags & ( ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsScientific ) ) == 0 )
		flags |= InputScalar_DefaultCharsFilter( data_type, format );
...
{
	// Hexadecimal input provided as a convenience but the flag name is awkward. Typically you'd use InputText() to parse your own data, if you want
	// to handle prefixes.
	const char* format = ( flags & ImGuiInputTextFlags_CharsHexadecimal ) ? "%08X" : "%d";
	return InputScalar( label, ImGuiDataType_S32, ( void* )v, ( void* )( step > 0 ? &step : NULL ), ( void* )( step_fast > 0 ? &step_fast : NULL ),
	                    format, flags );
}
...
static ImVec2 InputTextCalcTextSizeW( const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining, ImVec2* out_offset,
...
// Wrapper for stb_textedit.h to edit text (our wrapper is for: statically sized buffer, single-line, wchar characters. InputText converts between
// UTF-8 and wchar)
namespace ImStb
{
	...
	// When ImGuiInputTextFlags_Password is set, we don't want actions such as CTRL+Arrow to leak the fact that underlying data are blanks or
	// separators.
...
void ImGuiInputTextCallbackData::InsertChars( int pos, const char* new_text, const char* new_text_end )
{
	...
	if ( new_text_len + BufTextLen >= BufSize ) {
		...
		// Contrary to STB_TEXTEDIT_INSERTCHARS() this is working in the UTF8 buffer, hence the mildly similar code (until we remove the U16 buffer
		// altogether!)
		ImGuiContext& g                 = *GImGui;
                                      ...
                                      ImGuiInputSource input_source )
{
	...
	if ( apply_named_filters &&
		...
		// The libc allows overriding locale, with e.g. 'setlocale(LC_NUMERIC, "de_DE.UTF-8");' which affect the output/input of printf/scanf to use
		// e.g. ',' instead of '.'. The standard mandate that programs starts in the "C" locale where the decimal point is '.'. We don't really intend
		// to provide widespread support for it, but out of empathy for people stuck with using odd API, we support the bare minimum aka overriding
...
// FIXME: Ideally we should transition toward (1) making InsertChars()/DeleteChars() update undo-stack (2) discourage (and keep reconcile) or obsolete
// (and remove reconcile) accessing buffer directly.
                         ...
                         ImGuiInputTextCallback callback, void* callback_user_data )
{
	...
	// PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_FramePadding, ImVec2(0, 4));
	...
	if ( is_multiline ) {
		...
		ItemSize( total_bb, style.FramePadding.y + 4.f );
		...
		// Support for internal ImGuiInputTextFlags_MergedItem flag, which could be redesigned as an ItemFlags if needed (with test performed in
		// ItemAdd)
		ItemSize( total_bb, style.FramePadding.y + 4.f );
	...
	// We have an edge case if ActiveId was set through another widget (e.g. widget being swapped), clear id immediately (don't wait until the end of
	// the function)
	...
	if ( g.ActiveId == id ) {
		...
		// (For Tab and Enter: Win32/SFML/Allegro are sending both keys and chars, GLFW and SDL are only sending keys. For Space they all send all
		// threes)
		...
		// We ignore CTRL inputs, but need to allow ALT+CTRL as some keyboards (e.g. German) use AltGR (which _is_ Alt+Ctrl) to input certain
		// characters.
	...
	if ( g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id ) {
		...
		const int row_count_per_page  = ImMax( ( int )( ( inner_size.y - style.FramePadding.y + 4.f ) / g.FontSize ), 1 );
		...
		// Using Shortcut() with ImGuiInputFlags_RouteFocused flag to allow routing operations for other code (e.g. calling window trying to use
		...
		const bool is_cut =
			( Shortcut( ImGuiMod_Shortcut | ImGuiKey_X, id, shortcut_flags ) || Shortcut( ImGuiMod_Shift | ImGuiKey_Delete, id, shortcut_flags ) ) &&
			!is_readonly && !is_password && ( !is_multiline || state->HasSelection( ) );
		const bool is_copy = ( Shortcut( ImGuiMod_Shortcut | ImGuiKey_C, id, shortcut_flags & ~ImGuiInputFlags_Repeat ) ||
		                       Shortcut( ImGuiMod_Ctrl | ImGuiKey_Insert, id, shortcut_flags & ~ImGuiInputFlags_Repeat ) ) &&
		                     !is_password && ( !is_multiline || state->HasSelection( ) );
		const bool is_paste =
			( Shortcut( ImGuiMod_Shortcut | ImGuiKey_V, id, shortcut_flags ) || Shortcut( ImGuiMod_Shift | ImGuiKey_Insert, id, shortcut_flags ) ) &&
			!is_readonly;
		const bool is_undo = ( Shortcut( ImGuiMod_Shortcut | ImGuiKey_Z, id, shortcut_flags ) ) && !is_readonly && is_undoable;
		const bool is_redo = ( Shortcut( ImGuiMod_Shortcut | ImGuiKey_Y, id, shortcut_flags ) ||
		                       ( is_osx && Shortcut( ImGuiMod_Shortcut | ImGuiMod_Shift | ImGuiKey_Z, id, shortcut_flags ) ) ) &&
		...
		// We allow validate/cancel with Nav source (gamepad) to makes it easier to undo an accidental NavInput press with no keyboard wired, but
		...
		const bool is_cancel =
			Shortcut( ImGuiKey_Escape, id, shortcut_flags ) || ( nav_gamepad_active && IsKeyPressed( ImGuiKey_NavGamepadCancel, false ) );
	...
	if ( g.ActiveId == id ) {
		...
		// When using 'ImGuiInputTextFlags_EnterReturnsTrue' as a special case we reapply the live buffer back to the input buffer before clearing
		// ActiveId, even though strictly speaking it wasn't modified on this frame. If we didn't do that, code like InputInt() with
		// ImGuiInputTextFlags_EnterReturnsTrue would fail. This also allows the user to use InputText() with ImGuiInputTextFlags_EnterReturnsTrue
		// without maintaining any user-side storage (please note that if you use this property along ImGuiInputTextFlags_CallbackResize you can end
		// up with your temporary string object unnecessarily allocating once a frame, either store your string data, either if you don't then don't
		...
		if ( apply_edit_back_to_user_buffer ) {
			...
			if ( ( flags & ( ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit |
				...
				if ( event_flag ) {
					...
					// We have to convert from wchar-positions to UTF-8-positions, which can be pretty slow (an incentive to ditch the ImWchar buffer,
	...
	if ( !is_multiline ) {
		RenderNavHighlight( frame_bb, id );
		RenderFrame( frame_bb.Min, frame_bb.Max, ImColor( 25 / 255.f, 25 / 255.f, 25 / 255.f, 1.f ), true, style.PopupRounding );
		window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, ImColor( 50, 50, 50, 100 ), g.Style.FrameRounding );
	}
	...
	ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos + ImVec2( 5.f, 4.f ) : frame_bb.Min + style.FramePadding + ImVec2( 5.f, 2.f );
	...
	if ( render_cursor || render_selection ) {
		...
		// We are attempting to do most of that in **one main pass** to minimize the computation cost (non-negligible for large amount of text) + 2nd
		// pass for selection rendering (we could merge them by an extra refactoring effort)
...

		{
			...
			int line_count = 0;
			// for (const ImWchar* s = text_begin; (s = (const ImWchar*)wcschr((const wchar_t*)s, (wchar_t)'\n')) != NULL; s++)  // FIXME-OPT: Could
			// use this when wchar_t are 16-bit
		...
		if ( render_cursor && state->CursorFollow ) {
			...
			if ( is_multiline ) {
				...
				else if ( cursor_offset.y - ( inner_size.y - style.FramePadding.y + 4.f * 2.0f ) >= scroll_y )
					scroll_y = cursor_offset.y - inner_size.y + style.FramePadding.y + 4.f * 2.0f;
				const float scroll_max_y = ImMax( ( text_size.y + style.FramePadding.y + 4.f * 2.0f ) - inner_size.y, 0.0f );
		...
		if ( render_selection ) {
			...
			ImU32 bg_color =
				GetColorU32( ImGuiCol_TextSelectedBg, render_cursor ? 1.0f : 0.6f ); // FIXME: current code flow mandate that render_cursor is always
			...
			for ( const ImWchar* p = text_selected_begin; p < text_selected_end; ) {
				...
				if ( rect_pos.y < clip_rect.y ) {
					// p = (const ImWchar*)wmemchr((const wchar_t*)p, '\n', text_selected_end - p);  // FIXME-OPT: Could use this when wchar_t are
					// 16-bit p = p ? p + 1 : text_selected_end;
		...
		// We test for 'buf_display_max_length' as a way to avoid some pathological cases (e.g. single-line 1 MB string) which would make ImDrawList
		// crash.
		...
		if ( render_cursor ) {
			...
			ImVec2 cursor_screen_pos = ImFloor( draw_pos + cursor_offset - draw_scroll ) + ImVec2( 0.f, 2.f );
	...
	if ( is_multiline ) {
		// For focus requests to work on our multiline we need to ensure our child ItemAdd() call specifies the ImGuiItemFlags_Inputable (ref issue
		// #4761)...
		Dummy( ImVec2( text_size.x, text_size.y + style.FramePadding.y + 4.f ) );
		ImGuiItemFlags backup_item_flags = g.CurrentItemFlags;
		...
		// ...and then we need to undo the group overriding last item data, which gets a bit messy as EndGroup() tries to forward scrollbar being
		// active...
	...
	if ( label_size.x > 0 )
		RenderText( ImVec2( frame_bb.Min.x, frame_bb.Min.y - g.Font->FontSize - 3.f ), label );
...
#	ifndef IMGUI_DISABLE_DEBUG_TOOLS
	...
	Text( "CurLenW: %d, CurLenA: %d, Cursor: %d, Selection: %d..%d", state->CurLenA, state->CurLenW, stb_state->cursor, stb_state->select_start,
	      stb_state->select_end );
	Text( "undo_point: %d, redo_point: %d, undo_char_point: %d, redo_char_point: %d", undo_state->undo_point, undo_state->redo_point,
...
{
	// This check is optional. Suppose we have two color widgets side by side, both widgets display different colors, but both colors have hue and/or
	// saturation undefined. With color check: hue/saturation is preserved in one widget. Editing color in one widget would reset hue/saturation in
	// another one. Without color check: common hue/saturation would be displayed in all widgets that have hue/saturation undefined.
	// g.ColorEditLastColor is stored as ImU32 RGB value: this essentially gives us color equality check with reduced precision.
	// Tiny external color changes would not be detected and this check would still pass. This is OK, since we only restore hue/saturation _only_ if
	// they are undefined, therefore this change flipping hue/saturation from undefined to a very tiny value would still be represented in color
	// picker.
	ImGuiContext& g = *GImGui;
...
// With typical options: Left-click on color square to open color picker. Right-click to open option menu. CTRL-Click over input fields to edit them
...
bool ImGui::ColorEdit4( const char* label, float col[ 4 ], ImGuiColorEditFlags flags )
{
	...
	PushStyleVar( ImGuiStyleVar_::ImGuiStyleVar_FramePadding, ImVec2( 2, 2 ) );
	...
	flags |= ( g.ColorEditOptions & ~( ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_DataTypeMask_ | ImGuiColorEditFlags_PickerMask_ |
	...
	if ( !( flags & ImGuiColorEditFlags_NoSmallPreview ) ) {
		...
		SetCursorPos( ImVec2( GetContentRegionAvail( ).x - 23.f, ImGui::GetCursorPos( ).y - 18.f ) );

		const ImColor col_v4( col[ 0 ], col[ 1 ], col[ 2 ], alpha ? col[ 3 ] : 1.0f );

		if ( ColorButton( "##ColorButton", col_v4, flags, ImVec2( 15.f, 10.f ) ) ) {
		...
		if ( BeginPopup( "picker", ImGuiWindowFlags_::ImGuiWindowFlags_NoMove ) ) {
			if ( g.CurrentWindow->BeginCount == 1 ) {
				...
				const auto position = GetWindowPos( );
				const auto size     = GetWindowSize( );

				PushClipRect( position, position + size, false );
				ImGui::GetWindowDrawList( )->AddRect( position, position + size, ImColor( 50, 50, 50, 100 ), style.WindowRounding );
				PopClipRect( );
...
// (In C++ the 'float col[4]' notation for a function argument is equivalent to 'float* col', we only specify a size to facilitate understanding of
// the code.)
// FIXME: we adjust the big color square height based on item width, which may cause a flickering feedback loop (if automatic height makes a vertical
// scrollbar appears, affecting automatic width..)
// FIXME: this is trying to be aware of style.Alpha but not fully correct. Also, the color wheel will have overlapping glitches with (style.Alpha
// < 1.0)
bool ImGui::ColorPicker4( const char* label, float col[ 4 ], ImGuiColorEditFlags flags, const float* ref_col )
{
	...
	float square_sz   = GetFrameHeight( ) + 3.f;
	float bars_width  = square_sz - 7.f; // Arbitrary smallish width of Hue/Alpha picking bars
	...
	if ( flags & ImGuiColorEditFlags_PickerHueWheel ) {
		...
		for ( int n = 0; n < 6; n++ ) {
			...
			ShadeVertsLinearColorGradientKeepAlpha( draw_list, vert_start_idx, vert_end_idx, gradient_p0, gradient_p1, col_hues[ n ],
			                                        col_hues[ n + 1 ] );
		}
		...
		// Render SV Square
		draw_list->AddRectFilledMultiColor( picker_pos, picker_pos + ImVec2( sv_picker_size, sv_picker_size ), col_white, hue_color32, hue_color32,
		                                    col_white );
		...
		draw_list->AddRectFilledMultiColor( ImVec2( bar0_pos_x, bar0_line_y - 10 ), ImVec2( bar0_pos_x + bars_width, bar0_line_y ),
		                                    ImColor( 0.f, 0.f, 0.f, 0.f ), ImColor( 0.f, 0.f, 0.f, 0.f ), ImColor( 0.f, 0.f, 0.f, 0.6f ),
		                                    ImColor( 0.f, 0.f, 0.f, 0.6f ) );
		draw_list->AddRectFilledMultiColor( ImVec2( bar0_pos_x, bar0_line_y + 10 ), ImVec2( bar0_pos_x + bars_width, bar0_line_y ),
		                                    ImColor( 0.f, 0.f, 0.f, 0.f ), ImColor( 0.f, 0.f, 0.f, 0.f ), ImColor( 0.f, 0.f, 0.f, 0.6f ),
		                                    ImColor( 0.f, 0.f, 0.f, 0.6f ) );
		draw_list->AddLine( ImVec2( bar0_pos_x, bar0_line_y ), ImVec2( bar0_pos_x + bars_width, bar0_line_y ), ImColor( 1.f, 1.f, 1.f, 1.f ) );
	...
	if ( ImGui::Button( "copy", ImVec2( ImGui::GetContentRegionAvail( ).x / 2 - 12.f, 20 ) ) )
		memcpy( temp, col, components * sizeof( float ) );

	ImGui::SameLine( );

	if ( ImGui::Button( "paste", ImVec2( ImGui::GetContentRegionAvail( ).x - 22.f, 20 ) ) ) {
		memcpy( col, temp, components * sizeof( float ) );
		value_changed = true;
	}

	// Render cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)

	draw_list->AddRect( sv_cursor_pos, sv_cursor_pos + ImVec2( 2.f, 2.f ), user_col32_striped_of_alpha );
	draw_list->AddRect( sv_cursor_pos - ImVec2( 1.f, 1.f ), sv_cursor_pos + ImVec2( 2.f, 2.f ) + ImVec2( 1.f, 1.f ),
	                    ImColor( 25 / 255.f, 25 / 255.f, 25 / 255.f, 1.f ) );
	// draw_list->AddRect(sv_cursor_pos - ImVec2(2.f, 2.f), sv_cursor_pos + ImVec2(2.f, 2.f) + ImVec2(2.f, 2.f), ImColor(10 / 255.f, 10 / 255.f, 10 /
	// 255.f, 1.f));

	// draw_list->AddCircleFilled(sv_cursor_pos, circle_animation.AnimationData->second, user_col32_striped_of_alpha, 12);
	// draw_list->AddCircle(sv_cursor_pos, circle_animation.AnimationData->second + 1, ImColor(60 / 255.f, 60 / 255.f, 60 / 255.f), 12);
	// draw_list->AddCircle(sv_cursor_pos, circle_animation.AnimationData->second + 2, ImColor(10 / 255.f, 10 / 255.f, 10 / 255.f), 12);
	...
	if ( alpha_bar ) {
		...
		draw_list->AddRectFilledMultiColor( ImVec2( bar1_pos_x, bar1_line_y - 10 ), ImVec2( bar1_pos_x + bars_width, bar1_line_y ),
		                                    ImColor( 0.f, 0.f, 0.f, 0.f ), ImColor( 0.f, 0.f, 0.f, 0.f ), ImColor( 0.f, 0.f, 0.f, 0.6f ),
		                                    ImColor( 0.f, 0.f, 0.f, 0.6f ) );
		draw_list->AddRectFilledMultiColor( ImVec2( bar1_pos_x, bar1_line_y + 10 ), ImVec2( bar1_pos_x + bars_width, bar1_line_y ),
		                                    ImColor( 0.f, 0.f, 0.f, 0.f ), ImColor( 0.f, 0.f, 0.f, 0.f ), ImColor( 0.f, 0.f, 0.f, 0.6f ),
		                                    ImColor( 0.f, 0.f, 0.f, 0.6f ) );
		draw_list->AddLine( ImVec2( bar1_pos_x, bar1_line_y ), ImVec2( bar1_pos_x + bars_width, bar1_line_y ), ImColor( 1.f, 1.f, 1.f, 1.f ) );
...
bool ImGui::ColorButton( const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg )
{
	...
	const float default_size = GetFrameHeight( );
	const ImVec2 size( ( size_arg.x == 0.0f ? default_size : size_arg.x ) + 5.f, ( size_arg.y == 0.0f ? default_size : size_arg.y ) );
	const ImRect bb( window->DC.CursorPos, window->DC.CursorPos + size );
	...
	if ( ( flags & ImGuiColorEditFlags_NoBorder ) == 0 ) {
		off = -0.75f; // The border (using Col_FrameBg) tends to look off when color is near-opaque and rounding is enabled. This offset seemed like a
	...
	if ( ( flags & ImGuiColorEditFlags_AlphaPreviewHalf ) && col_rgb.w < 1.0f ) {
		...
		if ( col_source.w < 1.0f )
			RenderColorRectWithAlphaCheckerboard( window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32( col_source ), grid_step,
...
void ImGui::ColorTooltip( const char* text, const float* col, ImGuiColorEditFlags flags )
{
	...
	if ( ( flags & ImGuiColorEditFlags_InputRGB ) || !( flags & ImGuiColorEditFlags_InputMask_ ) ) {
		if ( flags & ImGuiColorEditFlags_NoAlpha )
			...
			Text( "#%02X%02X%02X%02X\nR:%d, G:%d, B:%d, A:%d\n(%.3f, %.3f, %.3f, %.3f)", cr, cg, cb, ca, cr, cg, cb, ca, col[ 0 ], col[ 1 ], col[ 2 ],
			      col[ 3 ] );
	} else if ( flags & ImGuiColorEditFlags_InputHSV ) {
		if ( flags & ImGuiColorEditFlags_NoAlpha )
...
bool ImGui::TreeNodeBehavior( ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end )
{
	...
	// Some alteration have subtle effects (e.g. toggle on MouseUp vs MouseDown events) due to requirements for multi-selection and drag and drop
	// support.
	...
	// We set ImGuiButtonFlags_PressedOnClickRelease on OpenOnDoubleClick because we want the item to be active on the initial MouseDown in order for
...
// This is basically the same as calling TreeNodeEx(label, ImGuiTreeNodeFlags_CollapsingHeader). You can remove the _NoTreePushOnOpen flag if you want
// behavior closer to normal TreeNode().
...
bool ImGui::Selectable( const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg )
{
	...
	const ImVec2 text_min = pos + ImVec2( 4.f, 0.f );
	...
	auto selected_text_animation = ImAnimationHelper( id, ImGui::GetIO( ).DeltaTime );
	selected_text_animation.Update( selected ? 4.f : -4.f, 1.f, 0.f, 1.f );

	auto hovered_text_animation = ImAnimationHelper( id + 1u, ImGui::GetIO( ).DeltaTime );
	hovered_text_animation.Update( hovered ? 4.f : -4.f, 1.f, 0.5f, 1.f );
...
// Tip: If your vertical size is calculated from an item count (e.g. 10 * item_height) consider adding a fractional part to facilitate seeing
// scrolling boundaries (e.g. 10.25 * item_height).
                     ...
                     int height_in_items )
{
	...
	clipper.Begin( items_count, GetTextLineHeightWithSpacing( ) ); // We know exactly our line height here so we pass it as a minor optimization, but
                   ...
                   int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 frame_size )
{
	...
	// This is currently not exposed in the public API because we need a larger redesign of the whole thing, but in the short-term we are making it
	// available in PlotEx().
	return idx_hovered;
}
                       ...
                       float scale_max, ImVec2 graph_size, int stride )
{
	...
	PlotEx( ImGuiPlotType_Lines, label, &Plot_ArrayGetter, ( void* )&data, values_count, values_offset, overlay_text, scale_min, scale_max,
	        graph_size );
}
...
{
	ImGuiPlotArrayGetterData data( values, stride );
	PlotEx( ImGuiPlotType_Histogram, label, &Plot_ArrayGetter, ( void* )&data, values_count, values_offset, overlay_text, scale_min, scale_max,
	        graph_size );
}
...
// Ideally we also want this to be responsible for claiming space out of the main window scrolling rectangle, in which case ImGuiWindowFlags_MenuBar
// will become unnecessary. Then later the same system could be used for multiple menu-bars, scrollbars, side-bars.
...
static bool IsRootOfOpenMenuSet( )
{
	...
	// doesn't have the _ChildWindow flag, and we rely on this IsRootOfOpenMenuSet() check to allow hovering between root window/popup and first child
	// menu. In the end, lack of ID check made it so we could no longer differentiate between separate menu sets. To compensate for that, we at least
	// check parent window nav layer. This fixes the most common case of menu opening on hover when moving between window content and menu bar.
	// Multiple different menu sets in same nav layer would still open on hover, but that should be a lesser problem, because if such menus are close
	// in proximity in window content then it won't feel weird and if they are far apart it likely won't be a problem anyone runs into.
...
bool ImGui::BeginMenuEx( const char* label, const char* icon, bool enabled )
{
	...
	// Sub-menus are ChildWindow so that mouse can be hovering across them (otherwise top-most popup menu would steal focus and not allow hovering on
	// parent menu) The first menu in a hierarchy isn't so hovering doesn't get across (otherwise e.g. resizing borders with
	...
	// BeginMenu() calls per frame. If somehow this is ever becoming a problem we can switch to use e.g. ImGuiStorage mapping key to last frame used.
	if ( g.MenusIdSubmittedThisFrame.contains( id ) ) {
		if ( menu_is_open )
			menu_is_open =
				BeginPopupEx( id, window_flags ); // menu_is_open can be 'false' when the popup is completely clipped (e.g. zero size display)
		else
	...
	if ( window->DC.LayoutType == ImGuiLayoutType_Horizontal ) {
		...
		PopStyleVar( );
		window->DC.CursorPos.x +=
			IM_FLOOR( style.ItemSpacing.x * ( -1.0f + 0.5f ) ); // -1 spacing to compensate the spacing added when Selectable() did a SameLine(). It
		...
		RenderArrow( window->DrawList, pos + ImVec2( offsets->OffsetMark + extra_w + g.FontSize * 0.30f, 0.0f ), GetColorU32( ImGuiCol_Text ),
		             ImGuiDir_Right );
	}
	if ( !enabled )
		EndDisabled( );
	...
	if ( window->DC.LayoutType == ImGuiLayoutType_Vertical ) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
	{
		...
		// The 'HovereWindow == window' check creates an inconsistency (e.g. moving away from menu slowly tends to hit same window, whereas moving
		// away fast does not) But we also need to not close the top-menu menu when moving over void. Perhaps we should extend the triangle check to a
		// larger polygon. (Remember to test this on BeginPopup("A")->BeginMenu("B") sequence which behaves slightly differently as B isn't a Child of
		// A and hovering isn't shared.)
	...
	if ( !enabled ) // explicitly close if an open menu becomes disabled, facilitate users code a lot in pattern such as 'if (BeginMenu("options",
...
static void ImGui::TabBarLayout( ImGuiTabBar* tab_bar )
{
	...
	for ( int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++ ) {
		...
		// and we cannot wait for the next BeginTabItem() call. We cannot compute this width within TabBarAddTab() because font size depends on the
		// active window.
...
static void ImGui::TabBarScrollToTab( ImGuiTabBar* tab_bar, ImGuiID tab_id, ImGuiTabBarSection* sections )
{
	...
	ImGuiContext& g = *GImGui;
	float margin = g.FontSize * 1.0f; // When to scroll to make Tab N+1 visible always make a bit of N visible to suggest more scrolling area (since
	                                  // we don't have a scrollbar)
...
bool ImGui::TabItemEx( ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags )
{
	...
	// We don't have CPU clipping primitives to clip the CloseButton (until it becomes a texture), so need to add an extra draw call (temporary in the
...
{
	// While rendering tabs, we trim 1 pixel off the top of our bounding box so they can fit within a regular frame height while looking "detached"
	// from it.
	ImGuiContext& g   = *GImGui;
...
#	endif
	...
	RenderTextEllipsis( draw_list, text_ellipsis_clip_bb.Min, text_ellipsis_clip_bb.Max, text_pixel_clip_bb.Max.x, ellipsis_max_x, label, NULL,
	                    &label_size );
