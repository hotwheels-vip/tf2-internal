...
inline ImFont* verdana_11 = {};
inline ImFont* verdana_bd_11 = {};
inline ImFont* first_icon_14 = { };
inline ImFont* second_icon_20 = { };
inline ImFont* icon_20 = { };
...
namespace ImGui
{
    ...
    IMGUI_API void          RenderSettingsPopup(const char* str_id, const std::function< void() >& function);

    IMGUI_API void          ImageWithText(const char* label, int* selected_module, int wanted_module, ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1), const ImVec4& tint_col = ImVec4(1, 1, 1, 1), const ImVec4& border_col = ImVec4(0, 0, 0, 0));
    ...
    //    BeginPopup/EndPopup, etc. where the EndXXX call should only be called if the corresponding BeginXXX function
    //    returned true. Begin and BeginChild are the only odd ones out. Will be fixed in a future update.]
    IMGUI_API bool          BeginChild(const char* str_id, const ImVec2& size = ImVec2(0, 0), bool border = false, ImGuiWindowFlags flags = 0, bool show_text = true);
    ...
    IMGUI_API bool Keybind(const char* label, KeyBind_t* key_data, bool has_style = true);
...
inline void RenderFadedGradientLine(ImDrawList* draw_list, const ImVec2& position, const ImVec2& size, const ImColor& color)
{
    draw_list->AddRectFilledMultiColor(ImVec2(position.x, position.y), ImVec2(position.x + size.x / 2, position.y + size.y),
        ImColor(color.Value.x, color.Value.y, color.Value.z, color.Value.w * 0.f), ImColor(color.Value.x, color.Value.y, color.Value.z, color.Value.w), ImColor(color.Value.x, color.Value.y, color.Value.z, color.Value.w), ImColor(242, 101, 252, 0));

    draw_list->AddRectFilledMultiColor(ImVec2(position.x + size.x / 2, position.y),
        ImVec2((position.x + size.x / 2) + size.x / 2, position.y + size.y), ImColor(color.Value.x, color.Value.y, color.Value.z, color.Value.w),
        ImColor(color.Value.x, color.Value.y, color.Value.z, color.Value.w * 0.f), ImColor(color.Value.x, color.Value.y, color.Value.z, color.Value.w * 0.f), ImColor(color.Value.x, color.Value.y, color.Value.z, color.Value.w));
}
